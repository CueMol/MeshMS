// Cross-check data_SESsphpat_convex (convex SES mesh, ext unused) against the
// Python golden <mol>.convex.txt for {2spheres, 3spheres, tetra, ArgArg}.
//
// The golden is the RAW MeshState after data_SESsphpat_convex (NO orient_faces, NO
...
// means the convex GEOMETRY (mod_seg_loop_cir / the per-atom slicing) differs --
// the failure print reports the first differing vertex/face/normal.
#include <array>
#include <cmath>
#include <cstdio>
#include <fstream>
...
void check_mol(const std::string& mol) {
  ...
  const std::string refpath =
      std::string(MESHMS_REF_DIR) + "/" + mol + ".convex.txt";
  ...
  MeshState st;
  data_SESsphpat_convex(st, g, di, dc, ds, dl, dp, &ext, 1.4, 0.5);
