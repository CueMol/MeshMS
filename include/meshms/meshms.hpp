#pragma once
// See docs/API.md for the complete public API reference.
...
//   Auto = mesh faithfully first, and ONLY if that yields a degenerate,
//          NaN-poisoned mesh (a strongly symmetric molecule such as fullerene)
//          retry with small deterministic Gaussian perturbations of the atom
//          centers until the mesh is finite. Clean molecules never trigger a
//          retry, so Auto is identical to None for them.
// NOTE: the jittered retry is std-lib-RNG based and therefore NOT reproducible
// across platforms (it does not match numpy's RNG); it applies ONLY to the
// one-shot path -- the cache (compute_rs_from_array / build_mesh_from_cache) is
// always faithful.
...
// xyzr[i-1]). `radius_probe` is the solvent probe radius Rp. This path is always
// faithful (jitter does not apply to the cache).
...
// recomputed from the fused geometry. The cache is always faithful: the "auto"
// symmetry jitter (see build_surface_from_array) does NOT apply here.
MeshResult build_mesh_from_cache(const std::shared_ptr<RSCache>& rs,
                                 double mesh_size, bool fuse = false);

// One-shot convenience: compute_rs_from_array + build_mesh_from_cache.
// With jitter == Jitter::None (or for any molecule that meshes cleanly) the
// result is bit-for-bit identical (verts/faces) to building from the equivalent
// .xyzr file. The default jitter == Jitter::Auto adds a degenerate-symmetry
// fallback: if the faithful mesh is NaN-poisoned it is silently re-meshed with
// small deterministic center perturbations (see the Jitter enum). The retry is
// std-lib-RNG based and not cross-platform reproducible; it never fires for a
// clean molecule, so Auto leaves all well-behaved inputs bit-identical to None.
MeshResult build_surface_from_array(
    const std::vector<std::array<double, 4>>& xyzr, double radius_probe,
    double mesh_size, bool fuse = false, Jitter jitter = Jitter::Auto);
